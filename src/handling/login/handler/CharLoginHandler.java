...
public class CharLoginHandler {
    ...
    public static final void login(final LittleEndianAccessor slea, final MapleClient c) {
        String pwd = GameConstants.GMS ? slea.readMapleAsciiString() : "";
        String login = slea.readMapleAsciiString();
        if (!GameConstants.GMS) {
            pwd = LoginCrypto.decryptRSA(slea.readMapleAsciiString());
        } else {
            if (c.isLocalhost()) { //must be fake acc, only 1
                login = ServerConstants.MASTER_LOGIN;
                pwd = ServerConstants.MASTER;
	...
	int shield = demon ? slea.readInt() : 0;
        ...
        } else if (jobType == JobType.Mercedes) { //diff
            if (!LoginInformationProvider.getInstance().isEligibleItem(gender, 0, jobType.type, face) || !LoginInformationProvider.getInstance().isEligibleItem(gender, 1, jobType.type, hair)
	...
	if (!GameConstants.GMS) {
	    slea.readShort(); //0 = no 2nd pw, 1 = set 2nd pw
	}
