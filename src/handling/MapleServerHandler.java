...
//      final byte ivSend[] = fixed ? new byte[]{44, (byte) 231, 77, 33} : serverSend;
        ...
        session.write(LoginPacket.getHello(ServerConstants.MAPLE_VERSION, serverSend, serverRecv, client));
//      System.out.println("GETHELLO SENT TO " + address + "; fixed = " + fixed);
 ...
 //                     sb.append(recv.toString()).append("\n").append(HexTool.toString((byte[]) message)).append("\n").append(HexTool.toStringFromAscii((byte[]) message));
   ...
   //       FileoutputUtil.log("HeaderLogs.txt", "\r\nPacket Used: " + header.name() + " Decimal Vaule: " + header.getValue() + " Hex Value: " + HexTool.getOpcodeToString(header.getValue()) + "\r\n");
        }  
        switch (header) {
            case PONG:
                c.pongReceived();
