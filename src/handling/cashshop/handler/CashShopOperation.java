...
public class CashShopOperation {
    ...
    public static void EnterCS(final int playerid, final MapleClient c) {
        ...
        if (mts) {
            ...
            c.getSession().write(MTSPacket.startMTS(chr));
...
//              c.getSession().write(CSPacket.confirmToCSInventory(item, c.getAccID(), c.getPlayer().getCashInventory().getSNForItem(item)));
            } else {
                c.getSession().write(CSPacket.sendCSFail(0xB1));
            }
